...
const numResults: number[] = [];    // number[] is called array type.
const textResults: string[] = [];  // string[] is called array type specify that it is array of string

type numOrString = number|string;  // creates our own type numOrString 
type Result = {val: number, timestamp: Date};

//alternative is interfaces, which allows us to define sturcture of an object.
...
// objects
function printResult(resultObj: Result) { //{ } defines structure of object, with parameters and thier type
...
// because we can only get value of input types and TS dont know type of num1Element. so we have to tell it
    ...
    printResult({val: result as number, timestamp: new Date()}); //type casting - casting result as number
...
const myPromise = new Promise<string>((resolve, reject) => { //to specify which type it will resolve
    setTimeout(() => {
